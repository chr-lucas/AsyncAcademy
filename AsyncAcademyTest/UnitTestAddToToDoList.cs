...
{
    //Test class to ensure that assignments are added to To-do list if due date has not passed - Hanna W
    [TestClass]
    public class UnitTestAddToToDoList
    {
        ...
        private async Task VerifyToDoAddition(AsyncAcademyContext context)
        {
            ...
            //Adding a new assignment, which should increase todo list count
            Assignment newAssignment = new Assignment
            {
                Id = 2, //place holder ID
                CourseId = 1, //place holder ID 
            ...
            //add new assignment to test database
            context.Assignment.Add(newAssignment);
            await context.SaveChangesAsync();

            //recount todo list by running ongetasync again and rebuilding todo list
            var result = await welcomeModel.OnGetAsync();
            int finalCount = welcomeModel.ToDoList.Count;
            
            //test if todo list was updated
            Assert.AreEqual(initialCount + 1, finalCount, $"To-Do list should include assignments prior to assignment due date.");
        }
