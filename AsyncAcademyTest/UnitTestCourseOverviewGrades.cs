...
{
    [TestClass]
    public class UnitTestCourseOverviewGrades
    {
        ...
        public async Task Setup()
        {
            ...
            _dbContextOptions = new DbContextOptionsBuilder<AsyncAcademyContext>()
                .UseInMemoryDatabase(databaseName: "AsyncAcademyTestDb3") // Test database name
                .Options;
        ...
        private ClassOverviewModel CreateClassOverviewModel(AsyncAcademyContext context, int courseId)
        {
            return new ClassOverviewModel(context)
            {
        ...
        public async Task GradesOnGraphAreCorrect() 
        {
            using (var context = new AsyncAcademyContext(_dbContextOptions))
            {
                ...
                foreach (int studentId in studentIds) 
                {
                    ...
                    List<Submission> submissionsForStudent = context.Submissions.Where(a => (a.UserId == studentId)).ToList();
                    int numNotNull = 0;
                    foreach (var submission in submissionsForStudent) 
                    {
